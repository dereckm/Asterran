...
{
    public class PythonLexer : IGenericLexer
    {
        public List<GenericToken> Tokenize(string code)
        {
            ...
            // Define Python token patterns
            var pattern = @"(?<Comment>#.*)" +
                          @"|(?<String>""""""[\s\S]*?""""""|'''[\s\S]*?'''|""[^""\\]*(?:\\.[^""\\]*)*""|'[^'\\]*(?:\\.[^'\\]*)*')" +
                          @"|(?<Number>\b\d+(?:\.\d+)?\b)" +
                          @"|(?<Operator>:=|==|!=|<=|>=|\+=|-=|=|\+|-|\*|/)" +
                          @"|(?<Keyword>\b(import|from|as|def|class|return|with|try|except|pass|if|elif|else|for|while|in|and|or|not|is|None|True|False)\b)" +
            ...
            foreach (Match match in regex.Matches(code))
            {
                ...
                else if (match.Groups["String"].Success)
                {
                    ...
                    {
                        stripped = val.Length >= 6 ? val.Substring(3, val.Length - 6) : val;
                    }
                    else if (val.StartsWith("\"") || val.StartsWith("'"))
                    {
                        stripped = val.Length >= 2 ? val.Substring(1, val.Length - 2) : val;
                    }
